...
-(nonnull QCloudServiceConfiguration *)buildConfiguration:(nonnull CosXmlServiceConfig *)config{
    ...
    // todo iOS不支持：SocketTimeout、port、IsDebuggable、SignInUrl、BucketInPath、DnsCache、
    configuration.endpoint = endpoint;
    configuration.signatureProvider = [CosPluginSignatureProvider makeWithFlutterCosApi:flutterCosApi secretId:config.permanentSecretId secretKey:config.permanentSecretKey];
...
- (void)preBuildConnectionBucket:(nonnull NSString *)bucket serviceKey:(nonnull NSString *)serviceKey completion:(nonnull void (^)(NSNumber * _Nullable, FlutterError * _Nullable))completion {
    //iOS不支持预连接
    completion(nil ,[self buildNotSupportFlutterError]);
}
